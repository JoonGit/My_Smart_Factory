...
namespace My_Smart_Factory.Controllers
{
    ...
    public class OutgoingInspController : Controller
    {
        ...
        public async Task<string> Update(OqcDto requestDto)
        {
            ...
            var oqc = await _dbContext.OutgoingInspModels
        ...
        public async Task<IActionResult> Read(string controlnumber)
        {
            ...
            var oqcList = await _dbContext.OutgoingInspModels
                    .Include(o => o.Inspector)
                    .Include(o => o.Confirmor)
            ...
            if (oqcList.Count == 0) {
                ...
                OutgoingInspModel model = await _oqcService.CreateDefultModel(controlnumber, inspector, confirmor);
                if(model != null) {
                    await _dbContext.OutgoingInspModels.AddAsync(model);
                    await _dbContext.SaveChangesAsync();
                }
                // db의 저장된 데이터를 전송
                // 저장 전 보내면 Id값이 저장된 후에 만들어 지기 때문에
                // db 수정 과정에서 에러 발행한다
                model = await _dbContext.OutgoingInspModels.Where(o => o.Controlnumber == controlnumber).FirstOrDefaultAsync();
